...
vec3 colorTemp(float temp, float intensity){
    float temp_index = ((temp - 1000.) / 100.)*3.;
  //  float alpha = mod(temp , 100.) / 100.0;
  	float alpha = intensity;
    
	vec3 color = intensity * interpolate_color(alpha, int(temp_index), int(temp_index+3.));
/*
    */
...
void main() {
  ...
  float present = curSample.a;
	...
	float stages = 6.;
	float stage = 10.; //mod(elapsedTime * .6, stages);
	...
	if(stage == 0.) {
		...
		// crazy color bounce
		gl_FragColor = vec4(mod(elapsedTime + position, 1.) * sin(mod(elapsedTime * 4., TWO_PI)), 1.);
	} else if(stage <= 3.) {
		// fast rising stripes
		//if(normal.z == 0.) {
			const float speed = .1;
...
*/
		...
		vec3 color = colorTemp(1000.0+r*10000.,1.);
