...
ROOT = os.path.join(HERE, 'todaysart', 'SharedData')
...
def main():
    ...
    scans = sorted(d for d in os.listdir(ROOT)
                   if d.startswith('scan-') and
                   d not in ('scan-closeup', 'scan-2137'))
    n_scans = len(scans)
    ...
    fcache = os.path.join(HERE, 'fmats_camcam.npz')
    ...
    gt_poses = np.load(os.path.join(HERE, 'gt_poses.npy'),
